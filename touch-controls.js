...
function layoutForGame(gameName) {
  if (gameName === "pinballGame") {
    return '<div class="tc-pad"><div class="tc-row">' +
      touchBtn("<-", "pb-left", "tc-wide") +
      touchBtn("LAUNCH", "pb-launch") +
      touchBtn("->", "pb-right", "tc-wide") +
      "</div></div>";
  }
  ...
  if (gameName === "typeSurvive") {
    if (typeof typeScreen !== "undefined" && typeScreen === "over") {
      return '<div class="tc-pad"><div class="tc-row">' +
        touchBtn("RETRY", "type-space", "tc-wide") +
        touchBtn("MENU", "type-back") +
        "</div></div>";
    }
    return "";
  }
...
function wantsPhoneKeyboard(gameName) {
  if (!isNonComputerDevice()) return false;
  if (gameName === "wordle") return typeof wordleScreen !== "undefined" && wordleScreen === "play";
  if (gameName === "typeSurvive") return typeof typeScreen !== "undefined" && typeScreen === "play";
  return false;
}

function dockPhoneKeyboard() {
  if (!mobileTextInput || !document.body.classList.contains("phone-keyboard")) return;
  var vv = window.visualViewport;
  if (!vv) {
    mobileTextInput.style.top = "";
    mobileTextInput.style.bottom = "10px";
    return;
  }
  mobileTextInput.style.bottom = "auto";
  mobileTextInput.style.top = Math.max(8, vv.offsetTop + vv.height - 54) + "px";
}

function focusMobileKeyboard(gameName) {
  if (!mobileTextInput) return;
  var show = wantsPhoneKeyboard(gameName);
  document.body.classList.toggle("phone-keyboard", show);
  if (!show) {
...
if (mobileTextInput) {
  mobileTextInput.addEventListener("input", function (event) {
    if (event.inputType === "deleteContentBackward") {
      if (activeGame === "wordle") handleWordleKey("backspace");
      mobileTextInput.value = "";
      return;
    }
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      else if (activeGame === "typeSurvive" && typeScreen === "play") handleTypeKey(ch);
    }
    if (wantsPhoneKeyboard(activeGame)) mobileTextInput.focus();
  });
  mobileTextInput.addEventListener("keydown", function (event) {
    if (activeGame === "wordle") {
      if (event.key === "Enter") {
        event.preventDefault();
        handleWordleKey("enter");
        if (typeof updateTouchControls === "function") updateTouchControls("wordle");
      }
  ...
  mobileTextInput.addEventListener("blur", function () {
    if (!wantsPhoneKeyboard(typeof activeGame !== "undefined" ? activeGame : "")) return;
    setTimeout(function () {
      if (wantsPhoneKeyboard(activeGame) && document.body.classList.contains("phone-keyboard")) {
        mobileTextInput.focus();
      }
    }, 50);
  });
...
if (window.visualViewport) {
  window.visualViewport.addEventListener("resize", dockPhoneKeyboard);
  window.visualViewport.addEventListener("scroll", dockPhoneKeyboard);
}
...
function updateTouchControls(gameName) {
  ...
  if ((gameName === "typeSurvive" || gameName === "wordle") && !document.body.classList.contains("phone-keyboard")) {
    document.body.classList.add("touch-type");
  }
}
