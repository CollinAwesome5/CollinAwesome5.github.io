...
function SIGame() {
    ...
    //  All sounds.
    this.sounds = null;
    this.highScoreKey = "spaceInvadersHighScore";
    this.highScore = 0;
...
SIGame.prototype.initSounds = function() {
    if (this.sounds) return;
    try {
        this.sounds = new Sounds();
        this.sounds.init();
        this.sounds.loadSound('shoot', 'sounds/sounds_shoot.wav');
        this.sounds.loadSound('bang', 'sounds/sounds_bang.wav');
        this.sounds.loadSound('explosion', 'sounds/sounds_explosion.wav');
    } catch (e) {
        this.sounds = { mute: true, playSound: function () {} };
    }
};

SIGame.prototype.loadHighScore = function() {
    try {
        var n = parseInt(localStorage.getItem(this.highScoreKey) || "0", 10);
        this.highScore = isNaN(n) ? 0 : n;
    } catch (e) {
        this.highScore = 0;
    }
};

SIGame.prototype.saveHighScore = function() {
...
function GameLoop(siGame) {
    if (typeof activeGame !== "undefined" && activeGame !== "spaceInvaders") {
        return;
    }
...
WelcomeState.prototype.draw = function(siGame, dt, ctx) {
    ...
    ctx.font="16px Arial";
    ctx.fillText("High score: " + siGame.highScore, siGame.width / 2, siGame.height/2 + 32); 
};

WelcomeState.prototype.keyDown = function(siGame, keyCode) {
    if(keyCode == KEY_SPACE) {
        //  Space starts the game.
        if (siGame.sounds && siGame.sounds.unlock) siGame.sounds.unlock();
        siGame.level = 1;
        siGame.score = 0;
        siGame.lives = 3;
        siGame.shotsFired = 0;
...
GameOverState.prototype.draw = function(siGame, dt, ctx) {
    ...
    ctx.fillText("High score: " + siGame.highScore, siGame.width / 2, siGame.height / 2 + 64);
};
...
PlayState.prototype.enter = function(siGame) {
    ...
    this.width = siGame.width;
    this.height = siGame.height;
    ...
    //  Create the invaders.
    var ranks = Math.floor(this.config.invaderRanks + 0.1 * limitLevel);
    var files = Math.floor(this.config.invaderFiles + 0.2 * limitLevel);
    var invaders = [];
...
//}
    ...
    for(var i=0; i<this.invaderFileCount; i++) {
...
PlayState.prototype.draw = function(siGame, dt, ctx) {
    ...
    info = "Score: " + siGame.score + ", High: " + siGame.highScore + ", Level: " + siGame.level;
    ctx.textAlign = "right";
...
Sounds.prototype.playSound = function(name) {
    if (this.mute === true || window.siteMuted || !this.sounds[name]) return;
    try {
        var shot = this.sounds[name].cloneNode(true);
        shot.volume = 0.5;
        var playPromise = shot.play();
...
Starfield.prototype.draw = function() {
	...
	for(var i=0; i<this.stars.length;i++) {
		var star = this.stars[i];
		var size = Math.max(2, Math.round(star.size));
