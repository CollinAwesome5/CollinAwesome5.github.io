...
function wsFallbackLevel() {
  return {
    tiles: ["l", "e", "t", "t", "e", "r", "s"],
    words: [
      { word: "letters", x: 0, y: 2, dir: "H" },
      { word: "letter", x: 0, y: 2, dir: "V" },
      { word: "steel", x: 2, y: 0, dir: "V" },
      { word: "reset", x: 4, y: 1, dir: "V" },
      { word: "tree", x: 1, y: 4, dir: "H" },
      { word: "rest", x: 0, y: 5, dir: "H" },
      { word: "see", x: 6, y: 2, dir: "V" },
      { word: "set", x: 3, y: 6, dir: "H" },
      { word: "let", x: 0, y: 2, dir: "H" },
      { word: "tee", x: 2, y: 6, dir: "V" }
    ]
  };
}
...
function wsChunkId(index) {
  return Math.floor(wsNormIndex(index) / WS_CHUNK_SIZE);
}

function wsGetLevel(index) {
  var c = wsChunkId(index);
  var chunk = wsChunks[c];
  if (!chunk) return null;
  return chunk[wsNormIndex(index) % WS_CHUNK_SIZE] || null;
}

function loadWsChunk(index, cb) {
  var c = wsChunkId(index);
  if (wsChunks[c]) {
    if (cb) cb(wsGetLevel(index));
    return;
  }
  var already = !!wsChunkLoading[c];
  if (!wsChunkLoading[c]) wsChunkLoading[c] = [];
  if (cb) wsChunkLoading[c].push(cb);
  if (already) return;
  var name = String(c + 1).padStart(3, "0");
  var req = new XMLHttpRequest();
  req.open("GET", "wordscapes-levels/" + name + ".json", true);
  req.onload = function () {
    try {
      wsChunks[c] = JSON.parse(req.responseText);
    } catch (e) {
      wsChunks[c] = [];
    }
    var waits = wsChunkLoading[c] || [];
    delete wsChunkLoading[c];
    for (var i = 0; i < waits.length; i++) waits[i](wsGetLevel(index));
  };
...
function wsEnsureExtraS(tiles) {
  var requiredS = 0;
  if (wsLevel && wsLevel.words) {
    for (var i = 0; i < wsLevel.words.length; i++) {
      var word = wsLevel.words[i].word;
      var count = 0;
      for (var j = 0; j < word.length; j++) if (word[j] === "s") count += 1;
      if (count > requiredS) requiredS = count;
    }
  }
  var haveS = 0;
  var out = tiles ? tiles.slice() : [];
  for (var k = 0; k < out.length; k++) if (out[k] === "s") haveS += 1;
  if (haveS < requiredS + 1) out.push("s");
  return out;
}
...
function startWordscapesLevel(index, keepProgress) {
  ...
  var level = wsGetLevel(wsLevelIndex);
  if (!level) {
    loadWsChunk(wsLevelIndex, function (got) {
      if (!got) {
        wsLevel = wsFallbackLevel();
        wsTiles = wsEnsureExtraS(wsLevel.tiles.slice());
        wsCoinWord = wsHardestWord();
        shuffleWsTiles();
        return;
      }
      startWordscapesLevel(wsLevelIndex, keepProgress);
    });
    loadWsChunk(wsLevelIndex + WS_CHUNK_SIZE);
    return;
  }
  loadWsChunk(wsLevelIndex + WS_CHUNK_SIZE);
  wsLevel = level;
  ...
  if (keepProgress && applyWsProgress(loadWsProgress())) {
    wsTiles = wsEnsureExtraS(wsTiles);
    shuffleWsTiles();
    saveWsProgress();
    return;
  }
...
function drawWsPlay(ctx) {
  ...
  ctx.fillText(word ? word.toUpperCase() : "", WS_W / 2, 428);

  if (wsMessage && performance.now() < wsMessageUntil) {
    ctx.font = "bold 16px sans-serif";
    ctx.fillStyle = "#ffffff";
    ctx.fillText(wsMessage, WS_W / 2, 454);
  }
