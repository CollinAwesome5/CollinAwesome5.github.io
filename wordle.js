...
function drawWordle() {
  if (!wordleCtx) {
    if (window.scheduleGameFrame) window.scheduleGameFrame(drawWordle, "wordle");
    else requestAnimationFrame(drawWordle);
    return;
  }
  if (typeof activeGame !== "undefined" && activeGame !== "wordle") {
    if (window.scheduleGameFrame) window.scheduleGameFrame(drawWordle, "wordle");
    else requestAnimationFrame(drawWordle);
    return;
  }
...
function handleWordlePointer(event) {
  if (typeof activeGame === "undefined" || activeGame !== "wordle") return;
  if (wordleScreen === "play" && typeof focusMobileKeyboard === "function") {
    focusMobileKeyboard("wordle");
  }
  var pos = wordleCanvasPos(event);
