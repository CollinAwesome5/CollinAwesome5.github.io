...
function wordleQuitPuzzle() {
  resetWordleGame();
  if (typeof refreshWordleTouch === "function") refreshWordleTouch();
}

function handleWordleKey(key) {
  if (key === "escape") {
    if (wordleScreen === "menu") {
      if (typeof setActiveGame === "function") setActiveGame("none");
    } else {
      wordleQuitPuzzle();
    }
    return;
  ...
  if (wordleScreen === "over") {
    ...
    else if (key === "backspace") wordleQuitPuzzle();
    return;
  }
  if (key === "backspace") {
...
function wordlePlayButtons() {
  return [
    { id: "leave", label: "LEAVE", x: 16, y: 10, w: 100, h: 36 },
    { id: "new", label: "NEW WORD", x: WORDLE_W / 2 - 70, y: 10, w: 140, h: 36 },
    { id: "letters", label: "LETTERS", x: WORDLE_W - 116, y: 10, w: 100, h: 36 }
  ];
}

function wordlePlayBtnAt(pos) {
  var buttons = wordlePlayButtons();
...
function wordleMenuLeaveBtn() {
  return { id: "leave", label: "LEAVE", x: 16, y: 10, w: 100, h: 36 };
}
...
function drawWordleMenu(ctx) {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  var leave = wordleMenuLeaveBtn();
  ctx.fillStyle = "rgba(0,0,0,0.45)";
  ctx.fillRect(leave.x, leave.y, leave.w, leave.h);
  ctx.strokeStyle = "#ffffff";
  ctx.lineWidth = 2;
  ctx.strokeRect(leave.x + 0.5, leave.y + 0.5, leave.w - 1, leave.h - 1);
  ctx.fillStyle = "#ffffff";
  ctx.font = "bold 13px sans-serif";
  ctx.fillText(leave.label, leave.x + leave.w / 2, leave.y + leave.h / 2);
...
function drawWordlePlay(ctx) {
  ...
  var playBtns = wordlePlayButtons();
  ...
  ctx.fillText(wordleLen + " LETTERS", WORDLE_W / 2, 60);
...
function handleWordlePointer(event) {
  if (typeof activeGame === "undefined" || activeGame !== "wordle") return;
  var pos = wordleCanvasPos(event);
  if (wordleScreen === "menu") {
    var leave = wordleMenuLeaveBtn();
    if (pos.x >= leave.x && pos.x <= leave.x + leave.w && pos.y >= leave.y && pos.y <= leave.y + leave.h) {
      if (typeof setActiveGame === "function") setActiveGame("none");
      return true;
    }
  }
  if (wordleScreen === "play" || wordleScreen === "over") {
    var action = wordlePlayBtnAt(pos);
    if (action === "leave" || action === "letters") {
      wordleQuitPuzzle();
      return;
    }
    if (action === "new") {
    ...
    if (wordleScreen === "over") startWordleGame(wordleLen);
    return;
  }
  var buttons = wordleMenuButtons();
  for (var i = 0; i < buttons.length; i++) {
...
wordleCanvas.addEventListener("pointerdown", handleWordlePointer);
wordleCanvas.addEventListener("click", handleWordlePointer);
