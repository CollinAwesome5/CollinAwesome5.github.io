...
function refreshTypeTouch() {
  if (typeof updateTouchControls === "function" && typeof activeGame !== "undefined" && activeGame === "typeSurvive") {
    updateTouchControls("typeSurvive");
  }
}
...
function updateTypeGame(now) {
  if (typeof activeGame !== "undefined" && activeGame !== "typeSurvive") {
    typeLastTime = now;
    if (window.scheduleGameFrame) window.scheduleGameFrame(updateTypeGame, "typeSurvive");
    else requestAnimationFrame(updateTypeGame);
    return;
  }
...
typeCanvas.addEventListener("pointerdown", function () {
  if (typeof activeGame === "undefined" || activeGame !== "typeSurvive") return;
  if (typeScreen === "play" && typeof focusMobileKeyboard === "function") {
    focusMobileKeyboard("typeSurvive");
  }
});
